...
export const getMirrorPairStatsData = async () => {
  ...
  const [rewardInfos, farmConfig, govStats, assetStats] = await Promise.all([
    rewardInfoPromise,
    farmConfigPromise,
    govStatsPromise,
    assetStatsPromise,
...
export const calculateMirrorPairStats = (mirrorData, pool, govConfig, govVaults, terraSwapPoolResponses) => {
  ...
  rewardInfos.reward_infos.forEach((item: any) => {
    ...
    const pair =
      pairStats[item.asset_token] ||
      (pairStats[item.asset_token] = createPairStats(0, item.asset_token, poolInfos, govWeight, totalWeight, govStats));
    const value = new BigNumber(uusd.amount).times(item.bond_amount).times(2).div(pairPool.total_share).toString();
    pair.tvl = value;
