...
export const getPylonPairStatsData = async (height) => {
  try {
    ...
    const [rewardInfo, farmConfig, pylonApy] = await Promise.all([
      rewardInfoPromise,
      farmConfigPromise,
      pylonApyPromise,
    ]);
...
const getPylonApy = async () => {
  try {
    const [govData, lpData] = await Promise.all([
      fetchData(PYLON_API_ENDPOINT + 'governance/v1/overview'),
      fetchData(PYLON_API_ENDPOINT + 'liquidity/v1/overview'),
    ]);
...
export const calculatePylonPairStats = (pylonStatsData, poolInfos, govVaults, govConfig, terraSwapPoolResponses) => {
  ...
  const pairs = {};
  pairs[contracts.pylonToken] = createPairStats(
    poolApr,
    contracts.pylonToken,
    poolInfos,
    govWeight,
    totalWeight,
    govApy,
