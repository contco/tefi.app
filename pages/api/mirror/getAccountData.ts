...
export const calculatePoolDetails = (
  listing,
  rewardsBalance,
  priceResult,
  stakedLpDetails,
...
export const getMirrorAccount = async (address: string) => {
  try {
    ...
    const rewardsBalance =
      stakingPool && stakingRewards ? balance[BalanceKey.REWARD](stakingPool, stakingRewards) : null;
      ...
      if (stakingPool && pairsList && stakingRewards) {
        ...
        if (longStakedBalance?.[listing.token] || stakeableBalance?.[listing.token]) {
          ...
          if (stakedLpDetails?.asset?.amount !== '0' || unstakedLpDetails?.asset?.amount !== '0') {
            ...
            const poolData = calculatePoolDetails(
              listing,
              rewardsBalance,
              priceResult,
              stakedLpDetails,
            ...
            poolList.push({
              symbol1: 'UST',
              symbol2: listing.symbol,
              lpName: `${listing.symbol}-UST LP`,
              stakedLp: stakedLpBalance ?? '0',
              stakeableLp: unStakedLpBalance ?? '0',
              rewardsSymbol: MIR,
              ...poolData,
            });
          }
    ...
    const account = {
      mirrorShortFarm: shortData,
      mirrorStaking: result,
      mirrorHoldings,
      airdrops: mirrorAirdrops,
      gov,
      total: { mirrorPoolSum, mirrorPoolRewardsSum, mirrorHoldingsSum, mirrorAirdropSum },
    };
    return account;
  } catch (err) {
    const emptyAccount = {
      ...
      airdrops: [],
      gov: null,
      total: { mirrorPoolSum: '0', mirrorPoolRewardsSum: '0', mirrorHoldingsSum: '0', mirrorAirdropSum: '0' },
    };
    return emptyAccount;
  }
};
