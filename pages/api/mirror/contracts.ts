...
const helpers = ({ whitelist }: ContractAddressJSON): ContractAddressHelpers => {
  ...
  const getSymbol = (token?: string) => (!token ? '' : token.startsWith('u') ? token : whitelist[token]?.symbol ?? '');

  const getIsDelisted = (token: string) => whitelist[token]?.status === 'DELISTED';

  const toAssetInfo = (token: string) =>
    token === UUSD ? { native_token: { denom: token } } : { token: { contract_addr: token } };
  ...
  const parseAssetInfo = (info: AssetInfo | NativeInfo) => {
    const token = 'native_token' in info ? info.native_token.denom : info.token.contract_addr;
