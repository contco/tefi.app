...
const DEFAULT_PYLON_SUM = {
  pylonHoldingsSum: '0',
  pylonPoolSum: '0',
  pylonAirdropSum: '0',
  pylonPoolRewardsSum: '0',
  gatewayRewardsSum: '0',
...
const getPylonHoldings = (price: number, accountDetails: any) => {
  if (accountDetails && accountDetails?.data?.mineBalance) {
    ...
    return {
      pylonHoldingsSum: value.toString(),
      pylonHoldings: [
        {
          symbol: PYLON_TOKEN_SYMBOL,
          name: PYLON_TOKEN_NAME,
          contract: contracts.token,
          balance: mineBalance.toString(),
          value,
  ...
  if (mineStakingData?.data) {
    ...
    if (stakedBalance !== 0) {
      ...
      const gov = {
        ...
        rewards: earned.toString(),
        value: stakedValue,
        price: price.toString(),
        rewardsValue,
        totalValue,
        apy: apyPercent?.toString() ?? '0',
      };
      return gov;
    }
  }
  ...
  if (minePoolData?.data) {
    ...
    if (stakedBalance || balance) {
      ...
      const pylonPool = [
        {
          symbol1: 'UST',
          symbol2: PYLON_TOKEN_SYMBOL,
          lpName: PYLON_UST_LP,
          stakeableLp: balance.toString(),
          stakedLp: stakedBalance.toString(),
          rewards: earned.toString(),
          rewardsValue,
          apr: apy?.toString() ?? '0',
          ...
          rewardsSymbol: PYLON_TOKEN_SYMBOL,
...
export const getPylonAccount = async (address: string) => {
  try {
    ...
    if (mineOverview) {
      ...
      const { pylonPoolSum, pylonPoolRewardsSum, pylonPool } = getMinePoolInfo(
        priceInUst,
        minePoolData,
        liquidityOverview?.data?.apy,
        lpValue,
      );
      const { pylonAirdropSum, pylonAirdrops } = await getPylonAirdrops(address, priceInUst, airdropData?.data);
